...
ldr2 <- bring_in_data("lowfat_dehydrated_rep2.csv")
ldr2 <- ldr2 %>% mutate(DateTime = with_tz(DateTime, tz="EST") + hours(1))
ldr2_act <- read_csv("C:/Users/mdm2000/OneDrive - USNH/PeroPhysiology/data/lowfat_dehydration_rep2/lowfat_dehydrated_rep2_activity.csv", n_max=dim(ldr2)[1])
...
lowfat_dehydration_noOL_rep2 <- cages[-c(masterlist_noDup),]
```



```{r rename and merge files}
#should only need this for rep1
...
baseline_temps <-
  rbind(mouse_metadata_rep1[mouse_metadata_rep1$DateTime %within% baseline_temp_rep1,],
        mouse_metadata_rep2[mouse_metadata_rep2$DateTime %within% baseline_temp_rep1,])
