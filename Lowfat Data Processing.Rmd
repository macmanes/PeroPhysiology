...
```{r import data rep 3 message=FALSE, warning=FALSE, echo=FALSE}
...
```{r import data rep 4 message=FALSE, warning=FALSE, echo=FALSE}

#this is the physiology data for the dehydration part of the lowfat experiment rep4

path <- "C:/Users/mdm2000/OneDrive - USNH/PeroPhysiology/data/lowfat_dehydration_rep4/"
ldr4 <- bring_in_data_lowfat_rep4("lowfat_dehydrated_rep4.csv")
ldr4 <- ldr4 %>% mutate(DateTime = with_tz(DateTime, tz="EST"))
ldr4_act <- read_csv("C:/Users/mdm2000/OneDrive - USNH/PeroPhysiology/data/lowfat_dehydration_rep4/lowfat_dehydrated_rep4_activity.csv", n_max=dim(ldr4)[1])

ldr4_temps <- bring_in_temps_lowfat_rep4("lowfat_temps_rep4.csv")
```
...
```{r manipulate data LOWFAT REP4}

################### ldr4

#make activity data non-cumulative
ldr4 <- ldr4 %>% 
  add_column(MOD_ADX = ldr4_act$MOD_ADX) %>%
  mutate(Activity = MOD_ADX - lag(MOD_ADX, default = 0))
...
for (DV in dependent_variables){
  print(DV)
    model <- lm(as.formula(paste0(DV, " ~ Animal_ID")), data = ldr4_cages)
  model.metrics <- augment(model) %>% dplyr::select(-.hat, -.sigma, -.fitted)
  summ <- model.metrics %>% filter(abs(.std.resid) > 2) %>% as.data.frame()
...
all_lowfat_data <- rbind(lowfat_dehydration_noOL_rep1,lowfat_dehydration_noOL_rep2, lowfat_dehydration_noOL_rep3,lowfat_dehydration_noOL_rep4)
...
lowfat_EE <- ggplot(data = lowfat_baseline,aes(x=as.POSIXct(time),y=EE_kJH)) +
  scale_x_datetime(date_breaks = "2 hours", date_labels = "%H:%M") +
  annotate("rect", xmin = as.POSIXct("1970-01-01 00:00:01", tz="UTC"), 
           xmax = as.POSIXct("1970-01-01 06:00:00", tz="UTC"), 
           ymin = -Inf, ymax = Inf, fill="grey84") +
  ...
  geom_point(data = lowfat_day1, aes(color="Day 1", alpha=0.2))+
  geom_point(data = lowfat_day2, aes(color="Day 2", alpha=0.2))+
  geom_point(data = lowfat_day3, aes(color="Day 3", alpha=0.2))+
  theme_classic()+
  ylim(0,1.5) +
  geom_smooth(method='gam', se = FALSE, aes(color = "Baseline"))+
  geom_smooth(data = lowfat_day1, method='gam', se = FALSE, aes(color = "Day 1"))+
  geom_smooth(data = lowfat_day2, method='gam', se = FALSE, aes(color = "Day 2"))+
  geom_smooth(data = lowfat_day3, method='gam', se = FALSE, aes(color = "Day 3"))+
...
lowfat_activity <- ggplot(data = lowfat_baseline,aes(x=as.POSIXct(time),y=Activity)) +
  scale_x_datetime(date_breaks = "2 hours", date_labels = "%H:%M") +
  annotate("rect", xmin = as.POSIXct("1970-01-01 00:00:01", tz="UTC"), 
           xmax = as.POSIXct("1970-01-01 06:00:00", tz="UTC"), 
           ymin = -Inf, ymax = Inf, fill="grey84") +
...
lowfat_water <- ggplot(data = lowfat_baseline,aes(x=as.POSIXct(time),y=H2Omg)) +
  scale_x_datetime(date_breaks = "2 hours", date_labels = "%H:%M") +
  annotate("rect", xmin = as.POSIXct("1970-01-01 00:00:01", tz="UTC"), 
           xmax = as.POSIXct("1970-01-01 06:00:00", tz="UTC"), 
           ymin = -Inf, ymax = Inf, fill="grey84") +
...
lowfat_temperatures/lowfat_activity/lowfat_EE/lowfat_water
